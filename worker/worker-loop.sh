...
build_pr_body() {
  ...
  local problem root_cause solution testing future_work issue_context
...
${issue_context}"
  ...
  [[ -n "${root_cause//[[:space:]]/}" ]] \
    || root_cause="The requested behavior was absent or incomplete on the authoritative base branch. No agent-authored root-cause narrative was available, so this fallback remains explicit about that evidence gap."
  [[ -n "${solution//[[:space:]]/}" ]] \
  ...
  [[ -n "${testing//[[:space:]]/}" ]] \
    || testing="The worker ran its configured verification and review gates before publication. Repository checks remain the authoritative final result."
...
publication_authorized() {
  ...
  issue_json=$(gh issue view "$issue_num" --repo "$REPO_SLUG" --json state,labels 2>/dev/null) || {
  ...
  if ! gh api "repos/${REPO_SLUG}/git/ref/${CURRENT_CLAIM_REF#refs/}" >/dev/null 2>&1; then
    PUBLICATION_BLOCK_REASON="atomic issue claim no longer exists"
    return 1
  fi
  return 0
}
...
cancel_issue_publication() {
  ...
  local branch_name="$2"
  local reason="$3"
  log_error "Publication canceled for issue #${issue_num}: ${reason}"
  gh issue edit "$issue_num" --repo "$REPO_SLUG" \
    --remove-label "squad:processing" 2>/dev/null || true
  gh issue comment "$issue_num" --repo "$REPO_SLUG" \
    --body "⏹️ Squad Worker ${WORKER_ID} canceled publication because ${reason}. No branch or pull request was published by this run." 2>/dev/null || true
...
Auto-committed by Squad Worker ${WORKER_ID} (copilot left changes unstaged)." 2>/dev/null || true
  ...
  local change_summary pr_body
  change_summary=$(generate_change_summary "$base_ref")
  if [[ -z "$PR_EXECUTIVE_SUMMARY" ]]; then
    log "No .squad/pr-summary.md found — using git-based summary"
  fi
  pr_body=$(build_pr_body "$issue_num" "$issue_title" "$issue_body" "$change_summary")

  if ! publication_authorized "$issue_num" "squad" "squad:processing"; then
    cancel_issue_publication "$issue_num" "$branch_name" "$PUBLICATION_BLOCK_REASON"
    return 1
  fi
  ...
  local issue_summary
  issue_summary=$(git log --format="- %s" "${base_ref}..HEAD" 2>/dev/null | head -10)
  local diffstat_short
  diffstat_short=$(git diff --stat "${base_ref}..HEAD" 2>/dev/null | tail -1)
...
Auto-committed by Squad Worker ${WORKER_ID}." 2>/dev/null || true
  ...
  if ! publication_authorized "$issue_num" "squad" "squad:revision" "squad:processing"; then
    cancel_issue_publication "$issue_num" "$branch_name" "$PUBLICATION_BLOCK_REASON"
    return 1
  fi
  ...
  revision_commits=$(git log --format="- %s" "${base_ref}..HEAD" 2>/dev/null | head -10)
  local revision_diffstat
  revision_diffstat=$(git diff --stat "${base_ref}..HEAD" 2>/dev/null | tail -1)
...
run_critic() {
  ...
  base_ref=$(trusted_base_ref)
  diff=$(git diff "${base_ref}..HEAD" 2>/dev/null | sed -n '1,1500p' || true)
  if [[ -z "$diff" ]]; then
    CRITIC_FAILURE_KIND="infrastructure"
    CRITIC_FEEDBACK="Critic found no diff between ${base_ref} and HEAD; refusing to approve without review input."
    log_error "$CRITIC_FEEDBACK"
    return 1
  fi
